...
wishlistController.addWishlistItem = async (req, res, next) => {
    try {
        const { price, image, mileage, year, make, model, url, zip, date } = req.body;

        const newWishlistItem = await pool.query('INSERT INTO Wishlist(price, image, mileage, year, make, model, url, zip, date) VALUES($1, $2, $3, $4, $5, $6, $7, $8, $9)', [price, image, mileage, year, make, model, url, zip, date]);
        ...
        return next({
            ...
            status: 400,
            message: { err: `Error in wishlistController.addWishlist : ${error}` },
        })
    }
}
...
wishlistController.deleteWishlistItem = async (req, res, next) => {
    try {
        ...
        const deleteWishListItem = await pool.query('DELETE FROM Wishlist WHERE id = $1', [id]);
        ...
        return next({
            ...
            status: 400,
            message: { err: `Error in wishlistController.deleteWishlistItem ${error}` },
        })    
    }
...
wishlistController.getWishlistItems = async (req, res, next) => {
    try {
        ...
        return next({
            ...
            status: 400,
            message: { err: `Error in wishlistController.getWishlistItems ${error}` },
        }) 
    }
}
